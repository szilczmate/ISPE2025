...
server <- function(input, output, session) {
  ...
  prep_data <- reactive({
    ...
    start_year_1 <- as.Date(paste0(input$startYear[1], "-01-01"))
    start_year_2 <- as.Date(paste0(input$startYear[2], "-01-01"))
    
    episodes <- episodes |>
      dplyr::filter(start_date >= start_year_1 & start_date <= start_year_2) |>
  ...
  #Kaplan-Meier plot stratified by the selected grouping variable
  output$survPlot <- renderPlot({
    ...
    if (input$groupVar == "none"){
      ...
      fit <- paste("Surv(time_months, disc) ~ drug +", input$groupVar)
    }
